...
router.post("/write", async (req, res, next) => {
  try {
    if (
      ...
      return res.status(522).json({ msg: "bad request: bad inputs" });
...
router.get("/single-blog/:_id", async (req, res, next) => {
  try {
    ...
    if (!thisBlog)
      return res.status(500).json({ msg: "bad request: no such blog exists" });
...
router.post("/by-user", async (req, res, next) => {
  try {
    ...
    if (!thisUser || !thisUser._id)
      throw new Error("bad request: no such user exists");
